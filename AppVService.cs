...
{    
    ...
    {        
        ...
        private void ClosePowerShellRunspace()
        {
            try
            {
                _runspace.Close();
            ...
            {
                LogMessage("The runspace failed to close a remote session.", EventLogEntryType.Error);
            }
            catch (InvalidRunspaceStateException)
            {
                LogMessage("The runspace is not in a state that allows it to be closed.", EventLogEntryType.Error);
            }
        }

        /// <summary>
        /// Starts the named pipe server, waits for messages from trhe VDA plugin
        /// </summary>
        [SuppressMessage("Microsoft.Design", "CA1031:DoNotCatchGeneralExceptionTypes", Justification = "Need to know if an error occured so the service doesn stop on unhandled exception")]
        ...
        private void CreatePowerShellRunspace()
        {
            ...
            initialState.AuthorizationManager = new AuthorizationManager("TheService");
            initialState.ImportPSModule(new[] {CommonAdminFunctions.ClientImportModulePath});
